...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String COL_FAVORITES_TITLE= "favorites_title";
    private static final String COL_FAVORITES_DURATION= "favorites_duration";
    private static final String COL_FAVORITES_ORDERBY= "favorites_orderby";
    private static final String COL_FAVORITES_URL= "favorites_url";
    ...
    private static final String CREATE_FAVORITES_TABLE = "CREATE TABLE IF NOT EXISTS " + TABLE_FAVORITES + " ( " +
            BaseColumns._ID+" INTEGER PRIMARY KEY, "+
            COL_FAVORITES_ORDERBY + " TEXT , " +
            COL_FAVORITES_TITLE + " TEXT , " +
            COL_FAVORITES_URL + " TEXT , " +
            COL_FAVORITES_DURATION + " TEXT" +
            ")";
    ...
    private static final String DROP_Favorites_TABLE = "DROP TABLE " + TABLE_FAVORITES;
    ...
    public void addFavorite(ItemModel service){
        ContentValues contentValues = new ContentValues();

        contentValues.put(COL_FAVORITES_DURATION, service.TrackDuration);
        contentValues.put(COL_FAVORITES_ORDERBY, service.UploadedBy);
        contentValues.put(COL_FAVORITES_TITLE, service.Title);
        contentValues.put(COL_FAVORITES_URL, service.Video_id);
    ...
    public ArrayList<ItemModel> getFavorite() {
        ArrayList<ItemModel> categories = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();

        Cursor cursor =db.query(TABLE_FAVORITES, null, null, null, null, null, null);
        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                categories.add(getFavorite(cursor));
            }
        }
        db.close();
        return categories;
    ...
    public ItemModel getFavorite(Cursor cursor){

        ItemModel item;
        item = new ItemModel(
                cursor.getString(cursor.getColumnIndex(COL_FAVORITES_TITLE)),
                "3:31",
                cursor.getString(cursor.getColumnIndex(COL_FAVORITES_ORDERBY)),
                "dd",
                cursor.getString(cursor.getColumnIndex(COL_FAVORITES_URL)),
                "",
                "",
                cursor.getString(cursor.getColumnIndex(COL_FAVORITES_ORDERBY)));
    ...
    public boolean isFavorite(String url){
        ...
        Cursor cursor = db.query(TABLE_FAVORITES, null, COL_FAVORITES_URL + " =? ", new String[]{String.valueOf(url)}, null, null, null);
    ...
    public void deleteFavorite(String url){
        ...
        db.delete(TABLE_FAVORITES, BaseColumns._ID + "=" + String.valueOf(getFavorite(url)), null);
    }

    public int getFavorite(String url){
        SQLiteDatabase db = getReadableDatabase();
        ...
        if (cursor.getCount()>0){
            ...
            Log.e("HH",cursor.getInt(cursor.getColumnIndex(BaseColumns._ID))+"");
