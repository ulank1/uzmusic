...
import uz.audio.Config.URLS;
import uz.audio.Network.VolleyUtils;
...
public class StreamUrlFetcher {
    ...
    private void requestStreamUrlUsingVolley(final String v_id) {
        ...
        //String url = "http://5.23.54.120/media/audio/Alisher_Zokirov_-_Yurak.mp3";
        Log.e("DuplicateTest"," requesting url for stream on:"+url) ;

        if(onStreamUriFetchedListener!=null){   // For Auto-next
            onStreamUriFetchedListener.onUriAvailable(v_id);
        }

        if(doBroadcast)         // for Manual play
            broadcastURI(v_id,file);
                    ...
                    public void onResponse(String result) {
                        ...
                        try {
                            ...
                            if (obj.getInt("status") == 200) {
                                ...
                                if(onStreamUriFetchedListener!=null){   // For Auto-next
                                    onStreamUriFetchedListener.onUriAvailable("http://5.23.54.120/media/audio/Alisher_Zokirov_-_Yurak.mp3");
                                }

                                if(doBroadcast)         // for Manual play
                                   broadcastURI("http://5.23.54.120/media/audio/Alisher_Zokirov_-_Yurak.mp3",file);
