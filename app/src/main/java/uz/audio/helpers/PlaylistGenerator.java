...
public class PlaylistGenerator {
    ...
    private void parsePlaylist1() {


        playlistItems = new ArrayList<>();
        Log.d("PlaylistGen"," response : "+playlistItems.size());

        try {
            ArrayList<PlaylistItem> playlistItems1 = Shared.playlistItems;
            for (int i = 0; i < Shared.playlistItems.size(); i++) {
                PlaylistItem item = playlistItems1.get(i);
                String v_id = item.getVideoId();
                String y_id = item.getYoutubeId();
                String _next_title = item.getTitle();
